...
The Phase 1 gate is open. Next pickup is S5. **2026-09-11: the formula
rename and the GitHub repo rename moved to a final Phase 3 (R1, R2) at
Sahil's request; roster fields move with them.**
...
| S4 | hexokit-site | `hexokit-landing` | S2 | L | D6 landing page + design iteration (§ Site shape). Hand-written, so it is on-brand from day one even while `/docs/` still reads "run-kit" | [hexokit-site#4](https://github.com/sahil87/hexokit-site/pull/4) | **merged** 2026-09-10 (fab change `lvnp`) — `/` on a StarlightPage splash wrapper; hero → install → seven feature cards (incl. a full-width operator-console card) → "Use any agent, untouched" → toolkit hexagon → desktop card → footer; assets OCR-triaged from Sahil's Desktop pool, tight card crops; copy per hexokit-site `docs/findings/landing-copy-study.md`; terminal island retained unmounted. Design accepted; #2 (`jwhx`) closed |
| S5 | hexokit-site | `hexokit-install-script` | S3 | S | D10: `/install` served from the same script with the product-first default; the `install-composition` Policy B text prepared for C1 | [hexokit-site#8](https://github.com/sahil87/hexokit-site/pull/8) | **shipped** 2026-09-11 (PR open, Copilot-reviewed) — hexokit-site fab change `d11j`. Deploy-time composition: `deploy.yml` fetches the upstream script unchanged, then `scripts/compose-install.mjs` replaces its last-line `main "$@"` anchor with `scripts/install-epilogue.sh` (no args → `run-kit`; `( main "$@" )` subshell; toolkit hint on the default path). `InstallOneLiner.astro` + `toolkit/install.md` flipped to hexokit.com/install product-first forms. C1 input: hexokit-site `docs/findings/install-composition-policy-b-patch.md`. shll.ai/install untouched until X2 |

### Phase 1 — brand surfaces, no renames (shll.ai live; repo, formula, roster untouched)

| # | Repo | Slug (suggested) | Depends on | Size | Scope | PR | Status |
|---|------|------------------|-----------|------|-------|----|--------|
| C1 | shll | `hexokit-banner-and-policy` | S4 accepted | S | **No rename of the standards (D14). Roster untouched (→ R1/R2).** Content only: `readme-extraction` §2 blockquote → "Part of [HexoKit](https://hexokit.com) — see all projects there"; `install-composition` Policy B install-docs location → hexokit.com; `config-home` example → `hexokit`; `versions.json` URL constant → hexokit.com (shll.ai kept as fallback); `shll skill` bundle prose. Leave every other `shll.ai` / "shll toolkit" mention for X4 | [shll#98](https://github.com/sahil87/shll/pull/98) | **PR open (draft)** — shll fab change `ttoa`; pipeline through review-pr done 2026-09-11; Copilot findings addressed |
| C3 | run-kit | `hexokit-brand-surfaces` | C1 | M | README H1/tagline (badge URLs stay `sahil87/run-kit` until R2); `docs/site/*` identity lines + `skill.md` H1; Electron `productName`/`artifactName` (D8, appId kept); `run-kit` cobra alias verb → `hexokit` (keep `run-kit` hidden alias one release); specs' present-tense identity lines; `fab/project/config.yaml` name. **No GitHub rename, no release-tooling change** (both → R1/R2) | | not started |
...
| C7 | fab-kit + wt, idea, tu, hop, sahil87 | `hexokit-banner-sweep` | C1 | S each | README banner via the standard; present-tense "run-kit" product mentions → HexoKit (`rk` verbs untouched); sahil87 profile table row + fix `brew install sahil87/tap/all` (no such formula). Repo links stay `sahil87/run-kit` until R2 | | **in progress** — tu: [tu#77](https://github.com/sahil87/tu/pull/77) draft, fab change `v2cu` (`260911-v2cu-hexokit-banner-sweep`), pipeline through review-pr done 2026-09-11, Copilot findings addressed (banner flip only; audit found no run-kit product mentions on tu's live surfaces); hop: [hop#70](https://github.com/sahil87/hop/pull/70) draft, fab change `mhhy` (`260911-mhhy-hexokit-banner-sweep`), pipeline through review-pr done 2026-09-11, Copilot approval recommended (banner flip only; sweep found no run-kit product prose — the 15 `run-kit` hits in docs/specs + docs/memory all name the `sahil87/run-kit` repo/workflow files, left for R2/D11). fab-kit: [fab-kit#668](https://github.com/sahil87/fab-kit/pull/668) draft, fab change `y50t` (`260911-y50t-hexokit-banner-sweep`), pipeline through review-pr done 2026-09-11, Copilot approved with 0 comments — README banner + 31 present-tense run-kit product mentions in kit skill prose → HexoKit, two product-named skill headings renamed (`## HexoKit (rk) Reference`, `## rk (HexoKit)`) with the pointer sweep into docs/specs + 4 memory files; `rk` substrate, `sahil87/run-kit`/`sahil87/tap/run-kit` identifiers, past-tense narrative, and the frozen 2.14.0 migration untouched (11 `run-kit` survivors in `src/kit`); Go `--help` strings and the wider memory narrative left as follow-ups; idea, sahil87 profile: not started; wt: fab change `2qsh` (`260911-2qsh-hexokit-banner-sweep`), PR [wt#59](https://github.com/sahil87/wt/pull/59) open (draft) 2026-09-11 — banner flip only; audit found no run-kit product mentions on wt's live surfaces; pipeline through review-pr done (2 Copilot comments fixed); ready to merge |
