...
S4 pipeline done, PR [hexokit-site#4](https://github.com/sahil87/hexokit-site/pull/4) open
draft, not yet merged (fab change `lvnp`) — awaiting Sahil's design acceptance,
which gates Phase 1. The earlier S4 attempt ([hexokit-site#2](https://github.com/sahil87/hexokit-site/pull/2),
fab change `jwhx`, standalone page) is still an open draft — presumably superseded
by #4; closing it is Sahil's call. Next pickup is S5 once S4 merges.
...
| S4 | hexokit-site | `hexokit-landing` | S2 | L | D6 landing page + design iteration (§ Site shape). Hand-written, so it is on-brand from day one even while `/docs/` still reads "run-kit" | [hexokit-site#4](https://github.com/sahil87/hexokit-site/pull/4) (draft) | **pipeline done, PR open** (fab change `lvnp`) — `/` is `src/pages/index.astro` on a StarlightPage splash wrapper (Sahil's pick over a standalone page; inherits S3's nav/head/footer, edits none of them); hero → install → six cards → "Nothing wraps your agent" → toolkit hexagon → desktop card → footer; assets OCR-triaged from Sahil's Desktop screenshot pool; terminal island retained unmounted for a later `/toolkit/` mount; review 30/30. Awaiting design acceptance. Earlier standalone-page attempt [hexokit-site#2](https://github.com/sahil87/hexokit-site/pull/2) (fab change `jwhx`) is still an open draft — presumably superseded by #4; closing it is Sahil's call |
